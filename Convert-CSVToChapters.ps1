...
$Chapters | ForEach-Object {
    ...
    $ChapterCSV = Import-Csv (Join-Path $ChaptersFolder "$($DVDLabel)_$($Chapter)\$($DVDLabel)_$($Chapter).csv")
    $ChapterCSV | %{ $CurrentChapter.AddChapterClip([HomeVideo_Clip]::new($_.Index,$_.Start,$_.End)) }
}
